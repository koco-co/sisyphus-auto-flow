# /using-autoflow 初始化流程重构 — 实施计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 重构 `/using-autoflow` 初始化流程，实现智能项目分类、已有项目深度扫描（7 维度）、新项目行业画像 + 调研方案推荐、配置验证，以及流程图重绘和 README 更新。

**Architecture:** 将 SKILL.md 重写为两条分支路径。分支 A（已有自动化项目）派 `project-scanner` Agent 深度通读项目代码，逐项确认后写入配置。分支 B（新项目）通过 5 个问题收集行业画像，派 `industry-researcher` Agent 网络调研后推荐 2-3 个完整技术方案，用户选择后试运行再全量生成。两条分支合流后进入共用步骤（仓库配置、连接配置、脚手架 + 配置验证）。行业信息持久化到 `autoflow-config.yaml`，后续 `/autoflow` 全流程读取。
...
| `templates/autoflow-config.yaml.j2` | autoflow-config.yaml 的 Jinja2 模板 |

### 修改文件

| 文件 | 变更 |
|------|------|
| `skills/using-autoflow/SKILL.md` | 完整重写：第零步智能分类 + 分支 A/B + 共用步骤 + 配置验证 |
| `skills/autoflow/SKILL.md` | 预检阶段读取 industry 配置并传递给下游 Agent |
...
| `scripts/scaffold.py` | 新增方案驱动脚手架 + autoflow-config.yaml 生成 |
...
## Task 4: 创建 autoflow-config.yaml Jinja2 模板

**Files:**
- Create: `templates/autoflow-config.yaml.j2`

- [ ] **Step 1: 创建模板文件**

```yaml
# autoflow-config.yaml — 由 /using-autoflow 自动生成
# 此文件记录项目配置，供 /autoflow 全流程读取
...
```bash
git add templates/autoflow-config.yaml.j2
git commit -m "feat: 新增 autoflow-config.yaml Jinja2 模板"
```

---

## Task 5: 重写 skills/using-autoflow/SKILL.md

这是核心任务。完整重写初始化流程，实现第零步智能分类 + 分支 A/B + 共用步骤 + 配置验证。

**Files:**
- Modify: `skills/using-autoflow/SKILL.md` (complete rewrite)

- [ ] **Step 1: 重写 SKILL.md 完整内容**

将 `skills/using-autoflow/SKILL.md` 完整替换为以下内容：
...
- 若检测到非 Python 测试代码（.java/.js/.ts 测试文件）：
  ```
  "注意：检测到非 Python 测试代码，AutoFlow 目前仅支持 Python 自动化测试。"
  ```
...
```bash
git add skills/using-autoflow/SKILL.md
git commit -m "feat: 重写 /using-autoflow — 智能分类 + 深度扫描/行业调研 + 方案推荐 + 配置验证"
```

---

## Task 6: 修改 skills/autoflow/SKILL.md — 增加行业配置读取

**Files:**
- Modify: `skills/autoflow/SKILL.md:43-51` (预检阶段的"项目配置读取"部分)

- [ ] **Step 1: 在预检阶段增加行业配置读取**

在 `skills/autoflow/SKILL.md` 的"**2. 项目配置读取**"段落末尾追加行业配置读取逻辑。找到以下内容：
...
在 `skills/autoflow/SKILL.md` 中找到完整 prompt 的 scenario-analyzer Agent 调用（约第 264-303 行区域），在 prompt 末尾追加：
...
在 `skills/autoflow/SKILL.md` 中找到 case-writer Agent 调用（约第 370-396 行区域），在 prompt 末尾追加：
...
```bash
git add skills/autoflow/SKILL.md
git commit -m "feat: /autoflow 预检增加行业配置读取，波次 2/3 传递行业上下文"
```
...
在 `agents/case-writer.md` 中找到"若 autoflow-config.yaml 不存在，使用默认的 httpx + pydantic 模式。"这行（第 52 行），在其后追加：
...
## Task 11: 修改 scripts/scaffold.py — 支持 autoflow-config.yaml 生成
...
- [ ] **Step 2: 在 generate_project 函数中追加 autoflow-config.yaml 渲染**
...
- [ ] **Step 3: 在 append_to_existing_project 函数中追加 autoflow-config.yaml 渲染**
...
git commit -m "feat: scaffold.py 支持 autoflow-config.yaml 模板渲染"
```
...
```bash
cd /Users/poco/Projects/sisyphus-autoflow
...
# 标题
$D shape add text --label "AutoFlow 工作流总览" --x 340 --y 20 -w 300 -h 40
...
# /autoflow 入口
$D shape add rounded --label "/autoflow <har-path>" --x 350 --y 770 -w 280 -h 50
...
找到 `Sisyphus--AutoFlow-1.1` 替换为 `Sisyphus--AutoFlow-1.2`。
...
- [ ] **Step 5: 更新配置参考 — 新增 autoflow-config.yaml industry 段**

找到"### autoflow-config.yaml"配置参考（约第 326 行），在现有示例之后追加：
...
```bash
ls -la agents/project-scanner.md agents/industry-researcher.md prompts/industry-assertions.md templates/autoflow-config.yaml.j2
```
...
```bash
grep -l "industry" skills/using-autoflow/SKILL.md skills/autoflow/SKILL.md agents/case-writer.md agents/scenario-analyzer.md agents/case-reviewer.md prompts/code-style-python.md scripts/scaffold.py README.md
```
...
```bash
cd /Users/poco/Projects/sisyphus-autoflow && python3 -m py_compile scripts/scaffold.py && echo "OK"
```
