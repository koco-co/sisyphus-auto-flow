...
用法：
    python -m sisyphus_auto_flow.parsers.har_parser <har_file> [--output <path>]
"""
...
from sisyphus_auto_flow.core.models.har import HarEntry, NormalizedHarRequest, NormalizedHarResult
from sisyphus_auto_flow.registry.module_registry import resolve_module
...
def _normalize_entries(source: str, total_entries: int, entries: list[HarEntry]) -> NormalizedHarResult:
    """将 HAR entry 列表归一化为统一结果。"""
    api_entries = [entry for entry in entries if _is_api_request(entry)]
    logger.info(f"过滤后剩余 {len(api_entries)} 条 API 请求")

    all_response_ids: dict[int, set[str]] = {}
    for index, entry in enumerate(api_entries):
        resp_body = _parse_body(entry.response.content.text)
        if isinstance(resp_body, dict):
            all_response_ids[index] = _extract_ids(resp_body)

    requests: list[NormalizedHarRequest] = []
    for seq, entry in enumerate(api_entries, start=1):
        ...
        mapping = resolve_module(path)
        module_name = mapping.module.value if mapping else None
        module_api_dir = mapping.api_dir if mapping else None
        module_description = mapping.description if mapping else None

        requests.append(
            NormalizedHarRequest(
                sequence=seq,
                method=entry.request.method,
                path=path,
                module=module_name,
                module_api_dir=module_api_dir,
...
def parse_har_file(
    ...
    *,
    provider: HarProvider | None = None,
) -> dict[str, Any]:
    ...
    if active_provider is None:
        ...
        active_provider = CustomHarProvider()

    result = active_provider.parse(har_path).model_dump()
...
def main(har_file: Path, output: Path | None, no_trash: bool) -> None:
    ...
    if output is None:
        project_root = Path(__file__).resolve().parents[3]
        output = project_root / ".data" / "parsed" / f"parsed_{har_file.stem}.json"
