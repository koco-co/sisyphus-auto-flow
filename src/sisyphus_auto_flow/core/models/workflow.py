...
class HarWorkflowManifest(BaseModel):
    ...
    acceptance_notice: str = Field(default="", description="终端验收通知")
    ...
    reference_sources: list[str] = Field(default_factory=list, description="参考源码与环境路径")
    validation_commands: list[str] = Field(default_factory=list, description="建议执行的验收命令")
