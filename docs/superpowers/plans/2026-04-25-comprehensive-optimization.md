...
skills/autoflow/SKILL.md       — 重写为极简编排（~200 行）
skills/using-autoflow/SKILL.md — 重写为极简编排（~250 行）
...
- [ ] **Step 5: scaffold.py 抽取重复的 autoflow-config 渲染**
...
### Task 6: 重写 skills/autoflow/SKILL.md 为极简编排

**Files:**
- Modify: `skills/autoflow/SKILL.md`

- [ ] **Step 1: 编写新 SKILL.md**

新的 `skills/autoflow/SKILL.md`：
...
5. **读取配置**：`repo-profiles.yaml` 和 `autoflow-config.yaml`，提取 repos、test_dir、test_types、industry 等
...
git commit -m "refactor: autoflow SKILL.md 重写为极简编排（682→130 行），按需加载 agents/*.md"
```

---

### Task 7: 重写 skills/using-autoflow/SKILL.md 为极简编排

**Files:**
- Modify: `skills/using-autoflow/SKILL.md`

- [ ] **Step 1: 编写新 SKILL.md**

新的 `skills/using-autoflow/SKILL.md`：
...
git commit -m "refactor: using-autoflow SKILL.md 重写为极简编排（866→120 行）"
```
...
Expected: "No hooks registered"（因为未配置 autoflow-config.yaml）

- [ ] **Step 3: 提交**

```bash
git add scripts/hooks.py
git commit -m "feat: hooks CLI run 子命令 — 支持从 autoflow-config.yaml 触发 hook"
```
