...
**Goal:** 为 `/autoflow` 和 `/using-autoflow` 两个 Skill 加入 Task 工具（TaskCreate/TaskUpdate/TaskList），实现全流程进度实时可视化。
...
|--------|------|---------------|
| Modify | `skills/autoflow/SKILL.md` | 追加 Task 工具权限 + 插入任务初始化 + 各阶段状态转换 |
| Modify | `skills/using-autoflow/SKILL.md` | 追加 Task 工具权限 + 插入任务初始化 + 分支动态更新 + 各步骤状态转换 |
| Modify | `README.md` | 核心特性表格新增一行 + 工作流总览新增任务可视化折叠段落 |

---

### Task 1: `/autoflow` — 追加 allowed-tools 权限

**Files:**
- Modify: `skills/autoflow/SKILL.md:6`

- [ ] **Step 1: 修改 allowed-tools**

在 `skills/autoflow/SKILL.md` 第 6 行，将：
...
git commit -m "feat: autoflow Skill 追加 TaskCreate/TaskUpdate/TaskList 工具权限"
```

---

### Task 2: `/autoflow` — 插入任务初始化段落

**Files:**
- Modify: `skills/autoflow/SKILL.md:9-10`（在"预检阶段"标题之前）

- [ ] **Step 1: 在第 9 行（`---` 分隔线之后、`## 预检阶段` 之前）插入任务初始化段落**

在 `skills/autoflow/SKILL.md` 中，紧接文件头部描述段落（第 8 行 `采用四波次编排流水线：解析、分析、生成、评审。`）之后、`---` 分隔线之前，插入以下内容：
...
```bash
git add skills/autoflow/SKILL.md
git commit -m "feat: autoflow 新增任务初始化段落 — 批量创建 6 个进度追踪任务"
```

---

### Task 3: `/autoflow` — 插入各阶段 TaskUpdate 状态转换

**Files:**
- Modify: `skills/autoflow/SKILL.md`（预检阶段、第一至四波次、验收报告各段）
...
在 `**验收报告**` 小标题之前（即 `读取 .autoflow/review-report.json 和 .autoflow/execution-report.json。` 之前）插入：
...
```bash
git add skills/autoflow/SKILL.md
git commit -m "feat: autoflow 各阶段插入 TaskUpdate 状态转换 — in_progress/completed"
```

---

### Task 4: `/using-autoflow` — 追加 allowed-tools 权限

**Files:**
- Modify: `skills/using-autoflow/SKILL.md:6`

- [ ] **Step 1: 修改 allowed-tools**

在 `skills/using-autoflow/SKILL.md` 第 6 行，将：
...
git commit -m "feat: using-autoflow Skill 追加 TaskCreate/TaskUpdate/TaskList 工具权限"
```

---

### Task 5: `/using-autoflow` — 插入任务初始化段落

**Files:**
- Modify: `skills/using-autoflow/SKILL.md:8-10`（在"第零步"标题之前）

- [ ] **Step 1: 在文件头部描述段落之后、`---` 分隔线之前插入任务初始化段落**

在 `skills/using-autoflow/SKILL.md` 中，紧接头部描述段落（第 6 行 `最终生成项目脚手架和 CLAUDE.md。`）之后、`---` 分隔线之前，插入以下内容：
...
```bash
git add skills/using-autoflow/SKILL.md
git commit -m "feat: using-autoflow 新增任务初始化段落 — 批量创建 6 个进度追踪任务"
```

---

### Task 6: `/using-autoflow` — 插入各步骤 TaskUpdate 状态转换 + 分支动态更新

**Files:**
- Modify: `skills/using-autoflow/SKILL.md`（第零步到第五步各段）
...
在文件最末尾（初始化摘要代码块 `下一步：执行 /autoflow <path-to.har>` 之后）插入：
...
```bash
git add skills/using-autoflow/SKILL.md
git commit -m "feat: using-autoflow 各步骤插入 TaskUpdate 状态转换 + 分支动态更新"
```
