...
为 sisyphus-autoflow 的两个核心 Skill（`/autoflow` 和 `/using-autoflow`）加入 Task 工具（TaskCreate / TaskUpdate / TaskList），实现全流程进度实时可视化。
...
| 6 | 脚手架生成 + 配置验证 | 生成项目脚手架 | 渲染 autoflow-config.yaml → 运行 scaffold.py → 生成 CLAUDE.md → smoke test → 输出初始化摘要 |
